...
public class ShadowDOM {
    ...
    public static  WebElement getElement(Config testConfig ,String[] selectors, WebElement ...element) {
        ...
        Browser.waitForJStoLoad(testConfig);     // Wait for Page Load complete
        ...
        if (element.length == 0)
        {
            ...
            selectorElement = testConfig.driver.findElement(By.cssSelector(selector));
        }
        else {
        	wait.until(ExpectedConditions.presenceOfElementLocated(By.cssSelector(selector)));
        ...
        //Get the shadow root
        root = (WebElement)((JavascriptExecutor)testConfig.driver).executeScript("return arguments[0].shadowRoot", selectorElement);
